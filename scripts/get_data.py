...
def get_imf_data(indicators, country_codes, start_date, end_date, freq="A", db="IFS"):
    ...
    url = f"https://dataservices.imf.org/REST/SDMX_JSON.svc/CompactData/{db}/{freq}.{country_codes}.{indicators}?startPeriod={start_date}&endPeriod={end_date}"
    print(url)
...
def get_wdi_data(indicators, country_codes, start_year=1980, end_year=2023):
    ...
    country_codes = ";".join(country_codes)
    url = f"https://api.worldbank.org/v2/country/{country_codes}/indicator/{indicators_url}?source=2"
    print(url)
    params = {
        "format": "json",
...
def get_fred_series(series_ids, start_date=None, end_date=None):
...
def create_plot(
    ...
    if secondary_y:
        ...
        ax2.set_ylabel(
            secondary_y_label or secondary_y[0], fontsize=12, color="lightgrey"
        )
...
def create_bar_plot(
    ...
    if plot_type == "line":
        ...
        data.plot(
            kind="bar",
            ax=ax,
            color=primary_colors[: len(data.columns)],
            width=bar_width,
        )
...
def thailand_ext_debt(file_path):
    ...
    df.rename(columns=indicators, inplace=True)
    df["Auslandsverschuldung in USD (links)"] = (
        df["liab_direct_investment"] + df["liab_portfolio"]
    )
    df["Auslandsverschuldung in THB (rechts)"] = (
        df["Auslandsverschuldung in USD (links)"] * df["thb_usd"]
    )

    df = df[
        ["Auslandsverschuldung in USD (links)", "Auslandsverschuldung in THB (rechts)"]
    ]
...
def oil_prices_iran_2026(file_path):
    """
    Ölpreise während der Iran-/Hormus-Krise 2026.
    Nutzt tägliche Brent- und WTI-Spotpreise aus FRED/EIA.
    """
    series = {
        "DCOILBRENTEU": "Brent",
        "DCOILWTICO": "WTI",
    ...
    # Diese Daten bewusst als Parameter/Kommentar pflegen,
    # weil politische Ereignisdaten je nach Quelle unterschiedlich abgegrenzt werden können.
...
if __name__ == "__main__":
    ...
    oil_prices_iran_2026(image_path + 'oil_prices_iran_2026.svg')
    plt.show()
